...
if object_id('temp.wb_gdp_change_year_pairs_05', 'U') is not null
drop table temp.wb_gdp_change_year_pairs_05;
...
where 1 = 1
    and (b.year - a.year) in (1, 2, 4, 8, 16, 32)
...
select 
    a.*
    , geography_code = case when d.geography_code is null or trim(d.geography_code) = '' then 'Other' else d.geography_code end
    , d.income_code
    , economy_start_year_end_year = concat(a.economy, ', ', a.start_year, '-', a.end_year)
    , log_start_gdp = log(a.start_gdp)
    , smallest_biggest_or_most_recent =
        case 
        when b.economy is not null then 'Smallest Increase'
        when c.economy is not null then 'Biggest Increase'
        when a.is_most_recent_year = 1 then 'Most Recent'
        else 'Other'
        end
into temp.wb_gdp_change_year_pairs_05
from temp.wb_gdp_change_year_pairs_03 a 
...
left join temp.wb_gdp_change_year_pairs_04 c on a.economy = c.economy and a.difference_in_years = c.difference_in_years and a.gdp_rate_of_change = c.max_gdp_rate_of_change
left join [WB].[clean_country_name_translation] d on a.economy = d.name
where 1 = 1
    and 
    (
        1 = 0
